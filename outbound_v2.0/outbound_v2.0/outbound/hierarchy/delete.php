...
<?php
	if($_POST)
	{
		...
		mysql_query("DELETE FROM is_hierarchy WHERE processid='$processid'",$connection);
	}
	$existingProcess = mysql_query("SELECT id,processname FROM is_process WHERE processtype=1",$connection);
	echo "<table width='100%' cellpadding='0' cellspacing='0'>";
	echo "<tr><td colspan='3'>&nbsp;</td></tr>";
	echo "<tr><td colspan='3' align='center'><span class='heading'>Delete Hierarchy</span></td></tr>";
	echo "<tr><td colspan='3'>&nbsp;</td></tr>";
	echo "<tr>
	...
	while($row = mysql_fetch_row($existingProcess))
	{
		if($i%2==0 && $i!=0)
		{
			$hierarchyQuery = mysql_query("SELECT b.firstname,b.lastname,b.id FROM is_hierarchy as a JOIN is_users as b ON(a.userid=b.userid AND a.level=0) WHERE a.processid='$row[0]'",$connection);
		...
		{
			$hierarchyQuery = mysql_query("SELECT b.firstname,b.lastname,b.id FROM is_hierarchy as a JOIN is_users as b ON(a.userid=b.id) WHERE a.processid='$row[0]' AND a.level!=0",$connection);
			while($name = mysql_fetch_row($hierarchyQuery))
			{
				$names .= $name[0]." ".$name[1].",";
				$userid .= $name[2].",";
