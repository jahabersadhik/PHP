...
if(!$_POST)
{
	...
	while($row = mysql_fetch_row($existingProcess))
	{
		if($i%2==0 && $i!=0)
		{
			$hierarchyQuery = mysql_query("SELECT b.firstname,b.lastname,b.id FROM is_hierarchy as a JOIN is_users as b ON(a.userid=b.userid) WHERE a.processid='$row[0]' AND a.level!=0",$connection);
