<?php
	...
	echo "
				<script language='javascript'>
					alert(\"Please Enter the Queue Number\");
					location.href='home.php?page=queues';
				</script>
			 ";
	$checkQueueNameResult = mysql_query("SELECT *FROM queues WHERE queueName='$queueName'");
	if(mysql_num_rows($checkQueueNameResult)==0)
	{
		if(count($_REQUEST['chk'])>0)
		{
			$extensionArr = implode(",",$_REQUEST['chk']);
			mysql_query("INSERT INTO queues VALUES('$queueName','$extensionArr','$process')");
			/*mysql_query("INSERT INTO availability VALUES('$queueName','')");
			mysql_query("INSERT INTO incomingavailability VALUES('$queueName','')");*/
