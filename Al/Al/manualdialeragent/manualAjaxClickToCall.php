...
#get the phone number from the posted form
	...
	$dialingIdQuery = mysql_query("select id from current where phone='$strExten' AND dialertype='0'");
	...
	if ($length && is_numeric($strExten)){
		...
		//mysql_query("update manualDialing set calledDateTime='$calledDateTime' where phone='".$_GET['toNumber']."'");						
		echo true;
		exit;
	}
}
