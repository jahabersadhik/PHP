...
for($i_Int=0;$i_Int<count($file2);$i_Int++)
{
	if(strstr($file2[$i_Int]," DLPN_")){
		...
		$name = trim($strDialPlan[1]);
		if(strstr($file2[$i_Int+1],"plancomment")){
			$strDialPlanName = explode("plancomment=",trim($file2[$i_Int+1]));
			$strPlanName[$name] = trim($strDialPlanName[1]);
		}
	}
	else
	{
		if(strstr($file2[$i_Int]," default")){
			$strDialPlan = explode(":",trim($file2[$i_Int]));
			$name = trim($strDialPlan[1]);
			if(strstr($file2[$i_Int+1],"plancomment")){
				$strDialPlanName = explode("plancomment=",trim($file2[$i_Int+1]));
...
{ 
	...
	$dlpname = $dialplanArr[$extenKey].'-'.$strPlanName[$dialplanArr[$extenKey]];
	xlsWriteLabel($xlsRow,4,$dlpname);
	$xlsRow++;
	$i++;
}
xlsEOF();
