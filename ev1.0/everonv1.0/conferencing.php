...
function editConf()
{
	...
	else
	{
		...
		var arr = num.split(',');
		document.getElementById('extNo').value = trim(arr[0]);
		document.getElementById('extPass').value = trim(arr[1]);
		...
		url=url+"?phn="+trim(arr[0])+"&pass="+trim(arr[1]);
...
function ajaxUpdate(mode)
{
	...
	if(mode=='add')
	{
		...
		url = url+"?extn="+trim(ext)+"&pass="+trim(pass)+"&m="+trim(opt1)+"&s="+trim(opt2)+"&i="+trim(opt3);
