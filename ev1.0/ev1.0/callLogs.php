...
curl_close($ch);
	...
	$startdate = addslashes($_REQUEST['txtStart']." 00:00:00");
	$enddate = addslashes($_REQUEST['txtEndt']." 23:59:59");
	if($_POST['calltype']=='any')
	{
		$condition = " AND (INSTR('$extstr',src) OR INSTR('$extstr',dst)) AND (calldate BETWEEN '$startdate' AND '$enddate') AND src!='' AND dst!=''";
	} 
	else if($_POST['calltype']=='in')
	{
...
$pager = new PS_Pagination($connection_cdr, $sql_select_calllog, $limit ,5, "param1=value1&param2=value2&calltype=".$_REQUEST['calltype']);
$rs = $pager->paginate();
...
function exportXLS()
{
	var URL = "export1.php?q=<?php echo $query; ?>&extstr=<?php echo $extstr;?>";
	location.href= URL;
}
</script>
</head>
...
if($_REQUEST['ext']!="")
{
	...
	echo "<tr >
			...
			<th class='stylefour' width='15%'>Source</td>	
			<th class='stylefour' width='15%'>Destination</td>
			<th class='stylefour' width='10%'>Call Type</td>
	...
	if(mysql_num_rows($rs)>0)
	{
		...
		while($row = mysql_fetch_array($rs))
		{
			...
			if($count%2==0 && $count!=0)
			{
				...
				echo "<td class='stylefourtext'>$calltype</td>";
