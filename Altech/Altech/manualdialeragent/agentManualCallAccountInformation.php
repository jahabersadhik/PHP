...
<?php
	...
	{
		$insertCallBackRecordQuery = mysql_query("select b.*,a.calledextension,a.processid from comment as a JOIN master as b ON(b.phone=a.phone) where a.id = '".$_REQUEST['id']."'");
		$accountRows = mysql_fetch_array($insertCallBackRecordQuery);
		mysql_query("INSERT INTO current VALUES(NULL,'".$accountRows['phone']."','".$accountRows['calledextension']."','0','".$accountRows['processid']."','1')");
		$acctQuery = "select * from current where dialedextension = '".$_REQUEST['id']."' AND dialertype=0 AND calltype=1";
		
	}
	else
	{
		$acctQuery = "select b.*,a.* from current as a JOIN master as b ON(b.phone=a.phone) where a.id = '".$_REQUEST['id']."' AND a.dialertype='0' AND a.calltype='0' ";
...
?>
		...
		<input type="hidden" name="dialType" value="<?php echo $_REQUEST['act'];?>"/>	
			...
			<!--<tr> 
             ...
             <tr> 
	        	 <td align=left> 
	        	 <label class="sublinks"> User Disposition</label>
	          	 </td>
	          	 <td class="NormalFont">
	          	 <label class="sublinks">
	          	<select name=userDisposition  style="width:170Px;" class="NormalTextBox">
				<option value="0" >-Select User Disposition-</option>
...
{	
	{
		...
		var parameters = "fromNumber="+fromNumber+"&toNumber="+toNumber+"&type=preview&id=<?php echo $_REQUEST['id']; ?>";  
		alert(parameters);
