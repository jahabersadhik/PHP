<?php
	$crmQuery = mysql_query("SELECT accountId,firstname,lastname FROM previewDialing GROUP BY accountId");
	$processQuery = mysql_query("SELECT processId,processName FROM process");
?>
<form method="POST" action="home.php?page=previewreport">
...
<?php
	if($_POST)
	{
		...
		$date = $_REQUEST['date'];
		$query = "SELECT *FROM previewDialing WHERE 1 AND calledExtension IS NOT NULL";
		if(trim($processId)!='-1')
