...
class PS_Pagination {
	...
	function renderNext($tag = 'Next &gt;&gt;') {
		...
		{
			if($_POST)
				$strstr = implode(",",$_POST['ext']);
			else
				$strstr = $_REQUEST['ext'];			
			return '<a href="' . $this->php_self . '?page=' . ($this->page + 1) . '&' . $this->append .'&ext='.$strstr.'&txtStart='.$_REQUEST['txtStart'].'&txtEndt='.$_REQUEST['txtEndt'].'&l='.$_REQUEST['l'].'">' . $tag . '</a>';
		} else {
			return $tag;
		}
	}
