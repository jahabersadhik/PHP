...
if($_GET['act']!="")
{
	$userModule = mysql_query("SELECT modules FROM users WHERE userId='$cust_cId' AND typeOfUser = 'admin'",$connection);
	$row_val = mysql_fetch_row($userModule);
	$moduleid= explode(",",$row_val[0]);
	
	$userTypeModule = mysql_query("SELECT modules FROM users WHERE userId='".$_POST['seluser']."'",$connection);
...
<script language="javascript">
		function redirect()
		{
			...
			document.customerprivileges.action = 'userPrivileges.php?act=change';	
