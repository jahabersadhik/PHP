...
for($i=0;$i<count($file);$i++){
	if(strstr($file[$i],"Category-")){
		$extensionArr = explode(":",$file[$i]);
		$extension = trim($extensionArr[1]);
	...
	if(strstr($file[$i],": fullname")){
		$fullnameArr = explode("=",$file[$i]);
		$fullname = $fullnameArr[1];
//		$fullname=substr(trim($file[$i]),29);
	...
	if(strstr($file[$i],": context")){
		$dialplanArr1 = explode("=",$file[$i]);
		$dialplan = $dialplanArr1[1];
//		$dialplan = substr(trim($file[$i]),28);
		if($dialplan!="")
		{
			$userArr[$l]['context']=$dialplan;
			$dialplanArr[$l] = $dialplan;
		}
		$l++;
	}
...
//	xlsWriteLabel($xlsRow,3,$secretArr[$extenKey]); 	
	xlsWriteLabel($xlsRow,3,$dialplanArr[$extenKey]);
	$xlsRow++;
	$i++;
}
xlsEOF();
